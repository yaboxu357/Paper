...
- Within each dataset and horizon, MSE and MAE are independently min-max normalized across completed runs. The retained run minimizes their equal-weight mean.

## Files

- `best_results_and_configs.csv`: canonical 36-row metric and full-configuration table.
...
- `all_completed_runs.csv`: every completed run recovered from logs.
- `run_recovered_best.sh`: reruns the 36 configurations; set `GPU` before launch.
- `SHA256SUMS`: integrity hashes for model code and recovery outputs.
- `dataset_manifest.csv`: sizes and SHA-256 hashes for the nine restored datasets.
