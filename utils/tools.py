...
def adjust_learning_rate(optimizer, epoch, args, scheduler=None, printout=True):
    ...
    if args.lradj == 'type1':
        ...
        lr_adjust = {epoch: scheduler.get_last_lr()}
    if epoch in lr_adjust.keys():
        lr = lr_adjust[epoch]
        if isinstance(lr, (list, tuple)):
            for param_group, group_lr in zip(optimizer.param_groups, lr):
            ...
            for param_group in optimizer.param_groups:
                param_group['lr'] = lr
            display_lr = lr
        if printout: print('Updating learning rate to {}'.format(display_lr))
