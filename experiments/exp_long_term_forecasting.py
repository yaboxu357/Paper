...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _select_optimizer(self):
        armor_names = ('channel_embedding', 'phase_embedding', 'joint_embedding')
        armor_params = []
        backbone_params = []
        for name, parameter in self.model.named_parameters():
            target = armor_params if any(key in name for key in armor_names) else backbone_params
            target.append(parameter)

        parameter_groups = [{'params': backbone_params, 'lr': self.args.learning_rate}]
        if armor_params:
            parameter_groups.append({
                'params': armor_params,
                'lr': self.args.learning_rate * self.args.armor_lr_scale,
            })
        model_optim = optim.AdamW(parameter_groups, weight_decay=self.args.weight_decay)
        return model_optim
    ...
    def train(self, setting):
        ...
        if self.args.lradj == 'TST':
            max_lrs = [group['lr'] for group in model_optim.param_groups]
