...
const outputDir = `${root}/outputs/detailed-results-truncated`;
const outputPath = `${outputDir}/experiment_results_detailed.xlsx`;
const previewPath = `${root}/.spreadsheet_work/detailed_results_preview.png`;
...
const trunc3 = (value) => isNumber(value) ? Math.trunc((Number(value) + Number.EPSILON) * 1000) / 1000 : null;
...
for (let rowIndex = 2; rowIndex < rows.length; rowIndex += 1) {
  for (const metricOffset of [0, 1]) {
    ...
    const distinct = [...new Set(entries.map((entry) => entry.value))].sort((a, b) => a - b);
...
sheet.getRange("A1:V48").values = rows;

sheet.mergeCells("A1:A2");
sheet.mergeCells("B1:B2");
for (let col = 2; col < 22; col += 2) {
...
for (let col = 2; col < 22; col += 2) {
  const left = String.fromCharCode(65 + col);
  const right = String.fromCharCode(65 + col + 1);
  sheet.mergeCells(`${left}48:${right}48`);
}

sheet.showGridLines = false;
sheet.freezePanes.freezeRows(2);
sheet.freezePanes.freezeColumns(2);
const all = sheet.getRange("A1:V48");
...
sheet.getRange("A1:V2").format.font = { name: "Times New Roman", size: 10, bold: true, color: "#000000" };
sheet.getRange("A3:A47").format.font = { name: "Times New Roman", size: 10, bold: true, color: "#000000" };
sheet.getRange("B3:B47").format.font = { name: "Times New Roman", size: 10, color: "#000000" };
sheet.getRange("C3:V47").format.numberFormat = "0.000";
sheet.getRange("A48:V48").format.font = { name: "Times New Roman", size: 10, bold: true, color: "#000000" };
sheet.getRange("A48:V48").format.borders = {
...
for (const [key, rank] of rankings.entries()) {
  ...
  if (rank === "best") {
    ...
    cell.format.font = { name: "Times New Roman", size: 10, color: "#000000" };
    cell.format.font.underline = "single";
  }
}

sheet.getRange("A1:A48").format.columnWidth = 12;
sheet.getRange("B1:B48").format.columnWidth = 17;
sheet.getRange("C1:V48").format.columnWidth = 9;
sheet.getRange("A1:V48").format.rowHeight = 20;
sheet.getRange("A1:V2").format.rowHeight = 24;
...
const preview = await workbook.render({ sheetName: "Sheet1", range: "A1:V48", scale: 1, format: "png" });
